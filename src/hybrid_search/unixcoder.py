...
class UniXcoder(nn.Module):
    def __init__(self, model_name):
        ...
        # The Microsoft wrapper passes a 3D encoder mask.  Force the eager
        # attention path so recent Transformers versions do not reinterpret it
        # as a modern 4D mask.
    ...
    def forward(self, source_ids):   
        ...
        # Recent Transformers expects a singleton head dimension for an
        # already-expanded mask; the pairwise encoder-only mask is unchanged.
        if getattr(self.model.config, "_attn_implementation", "") in {"eager", "sdpa", "flash_attention_2"}:
