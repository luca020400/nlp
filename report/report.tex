...
The tooling supports indexing, searching, and benchmarking with hit rate, MRR, recall, and precision metrics.
...
Code can be found at \url{https://github.com/luca020400/nlp}.
...
The system then scores and ranks the results, returning the top matches to the user, along their source locations.
...
Python and Java have different syntax, but both become the same type of object after extraction, minus the code body itself that is stored as is.
...
TF-IDF is relatively cheap, gives an interpretable result, and is good at exact matching.
...
\noindent
This is sufficient for scenarios where the focus is on function level search, as we don't perform any kind of full program analysis.

The extraction process does not try to parse every possible language feature, we picked a subset of the language that is sufficient for function search.
...
The IR decomposes identifiers at underscores and other non-word characters and exposes the resulting tokens to TF-IDF.

\subsection{Semantic encoding}
\label{subsec:semantic}

The semantic component supports two pretrained encoders.
\texttt{codebert} is a RoBERTa-based model trained on paired natural-language and programming-language data \cite{feng2020codebertpretrainedmodelprogramming},
while \texttt{unixcoder} is a RoBERTa-based model trained for unified cross-modal code understanding and generation \cite{guo2022unixcoderunifiedcrossmodalpretraining}.
...
\bigskip
Both models are exposed through the shared interface ``\texttt{SemanticEncoder}'', so the rest of the code does not depend on which encoder is in use.
...
\noindent
The indexed semantic vectors and the query embedding are also explicitly L2-normalized, therefore their dot product is another cosine similarity.
...
Instead of normalizing and weighting the different similarity scores, RRF treats retrieval as a voting problem based on the relative rank order.
...
By using reciprocal rank, top-ranked candidates receive more weight than lower-ranked ones, while the smoothing constant \(k\) reduces
...
\begin{center}
  ...
  \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{0.95in} >{\raggedright\arraybackslash}X}
    ...
    Metric & Meaning \\
    \midrule
    Recall@1 & Mean fraction of relevant functions retrieved at rank one. \\
    Recall@3 & Mean fraction of relevant functions retrieved in the first three results. \\
...
Using the UniXcoder, the best model found in this experiment, the following queries fail to fetch the expected function in the top three:
...
The benchmark is small, and the queries sometimes reuse words, or parts of words, from identifiers or documentation.
...
\begin{itemize}
  \item adding support for more programming languages via new extractors
  \item using a larger and more diverse benchmark set
  \item experimenting with different values of the hybrid weight, weighted RRF, or other fusion techniques
  \item fine tuning or adapting the encoders to the specific IR format
\end{itemize}
